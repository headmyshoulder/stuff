...
#define SET( name, var, type ) assert( vm.count( name##_LONG ) > 0 ); var = vm[ name##_LONG ].as< type >();
#define MAKE_MUTATION_STRATEGY( n ) boost::make_shared< mutation_strategy_##n >( argumentsCount(), mutation_strategy_arguments( weight(), crossover() ) )
...
CmdLine::CmdLine()
: _weight( 0 ), _crossover( 0 ), _populationSize( 0 ),
_maxGenerations( 0 ), _minimize( true ),
_processorsCount( 0 )
{
	notice( std::cout );
}
...
{
	std::ostringstream o;
	o << "Command line usage: " << std::endl << (std::string&)_description << std::endl;
	return o.str();
}
...
bool CmdLine::process( int argc, char* argv[] )
{
	try
	{
		...
		desc.add_options()
		( ARGS( COMMAND_LINE_HELP            ), "shows this message" )
		( ARGS( WEIGHT                       ), po::value< double >()->required(), "differential evolution weight factor in the range [0,2]" )
		( ARGS( CROSSOVER                    ), po::value< double >()->required(), "differential evolution crossover factor in the range [0,1]" )
		( ARGS( POPULATION_SIZE              ), po::value< size_t >()->required(), "population size" )
		( ARGS( MAX_GENERATIONS              ), po::value< size_t >()->required(), "max generations" )
		( ARGS( MUTATION_STRATEGY            ), po::value< size_t >()->default_value( 1 ), "mutation strategy - an integer between 1-5 to specify one of the 5 mutation strategies available" )
		( ARGS( SELECTION_STRATEGY           ), po::value< size_t >()->default_value( 1 ), "selection strategy:\n- 1: \"best parent child selection\"\n- 2: \"tournament selection\"" )
		( ARGS( MINIMIZE                     ), po::value< bool >()->required(), "0 to maximize, 1 to minimize" )
		( ARGS( CONFIG_FILE                  ), po::value< std::vector< std::string > >(), "configuration file" )
		( ARGS( PROCESSORS_COUNT             ), po::value< size_t >()->required(), "number of processors to run in parallel" )
		( ARGS( CONSTRAINTS                  ), po::value< std::vector< std::string > >()->required(), "constraints - there can be any number of constraints, one per variable. Format: \"type;min;max\", where min and max are floating point values, and type can be:\n"
																									   "- real\n"
																									   "- int\n"
																									   "- set\n"
																									   "- boolean")
		( ARGS( VARIABLES_COUNT              ), po::value< size_t >()->default_value( 20 ), "number of variables per individual - it is usually much higher than the number of variables required by the objecive function" )
		( ARGS( DEF_CONSTRAINT_MIN ), po::value< double >()->default_value( -1e6 ), "default min constraint - used for constraints that are not set on the command line" )
		( ARGS( DEF_CONSTRAINT_MAX ), po::value< double >()->default_value( 1e6 ), "default max constraint - used for constraints that are not set on the command line" )
		( ARGS( FUNCTION_TO_OPTIMIZE         ), po::value< size_t >()->required(), "function to optimize:\n"
																				   "- 1: -(x^2) [max: 0]\n"
																				   "- 2: x^2 [min: 0]\n"
		...
		{
			// now loading the arguments from the configuration file(s) - there can be any number of them
			if( vm.count( CONFIG_FILE_LONG ) > 0 )
			{
				std::vector< std::string > configFiles( vm[ CONFIG_FILE_LONG ].as< std::vector< std::string > >() );

				for( std::vector< std::string >::size_type n = 0; n < configFiles.size(); ++n )
				{
					std::ifstream ifs( configFiles[ n ].c_str() );
					if( ifs )
						po::store( po::parse_config_file( ifs, desc ), vm );
					else
						throw CmdLineException( ( boost::format( "Couldn't open configuration file: \"%1%\"") % configFiles[ n ] ).str() );
				}
			}
			...
			if( weight() < MIN_WEIGHT || weight() > MAX_WEIGHT )
				throw CmdLineException( ( boost::format( "Weight %1% out of range - it must be between [%2%, %3%]" ) % weight() % MIN_WEIGHT % MAX_WEIGHT ).str() );

			if( crossover() < MIN_CROSSOVER || crossover() > MAX_CROSSOVER )
				throw CmdLineException( ( boost::format( "Crossover %1% out of range - it must be between [%2%, %3%]" ) % crossover() % MIN_CROSSOVER % MAX_CROSSOVER ).str() );

			assert( vm.count( CONSTRAINTS_LONG ) > 0 ); 
			_constraints = boost::make_shared< de::constraints >( vm[ CONSTRAINTS_LONG ].as< std::vector< std::string > >(), _argumentsCount, argumentsDefConstraintMin, argumentsDefConstraintMax );

			assert( vm.count( FUNCTION_TO_OPTIMIZE_LONG ) > 0 ); 
			_functionToOptimize = boost::make_shared< FunctionToOptimize >( vm[ FUNCTION_TO_OPTIMIZE_LONG ].as< size_t >() );

			switch( selectionStrategyIndex )
			{
			case 1:
				_selectionStrategy = boost::make_shared< best_parent_child_selection_strategy >();
				break;
			case 2:
				_selectionStrategy = boost::make_shared< tournament_selection_strategy >();
				break;
			default:
				throw CmdLineException( ( boost::format( "Invalid selection strategy: %1%" ) % _selectionStrategy ).str() );
			}
			...
			case 5:
				...
				break;
			default:
				throw CmdLineException( ( boost::format( "Invalid mutation strategy: %1%" ) % _mutationStrategy ).str() );
...
FunctionToOptimize::FunctionToOptimize( size_t index )
: _index( --index )
{
}
...
objective_function_ptr FunctionToOptimize::operator()() const
{
	if( _index < sizeof( _functions ) / sizeof( objective_function_ptr ) )
		...
		throw CmdLineException( ( boost::format( "Function to optimize index not valid: %1%") % (_index + 1 ) ).str() );
}
