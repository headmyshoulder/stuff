...
void init(vector<Node> &initSequence)
{
    ...
    Operation varY( (unsigned int) 1, string("Y") );
    Operation varZ( (unsigned int) 2, string("Z") );
    ...
    // and we add the variables
    initSequence.push_back( varX );
    initSequence.push_back( varY );
    initSequence.push_back( varZ );
...
public:
    ...
    struct context
    {
        std::vector< double > x1 , x2 , x3 , y;
    };

    typedef eoParseTree<FitnessType, Node> EoType;

    static void createTestData( size_t n , context &c )
    {
        std::mt19937 rng;
        ...
        for( size_t i=0 ; i<n ; ++i )
        {
            ...
            double x2 = dist( rng );
            double x3 = dist( rng );
            c.y[i] = x1 * x1 * x1 + 1.0 / 10.0 * x2 * x2 + 3.0 / 4.0 * ( x3 - 4.0 ) + 1.0;
            c.x1[i] = x1;
            c.x2[i] = x2;
            c.x3[i] = x3;
        }
    ...
    void operator()(EoType &_eo)
    {
        ...
        vector< double > input(3); // the input variable(s)
        double output(0.);
        double target;
        FitnessType fitness;
        ...
        double fit=0;
        for( size_t i=0 ; i<m_c.x1.size() ; ++i )
        {
            input[0] = m_c.x1[i];
            input[1] = m_c.x2[i];
            input[2] = m_c.x3[i];
    ...
    RegFitness(eoValueParam<unsigned> &_generationCounter, vector< Node > &initSequence, Parameters &_parameter , const context &c )
        : eoEvalFunc<EoType>(), generationCounter(_generationCounter), parameter(_parameter) , m_c( c )
    {
        init(initSequence);
        best[NORMAL] = 1000;
        tree= "not found";
